...
class NghSampler2 (nn.Module):
    ...
    def forward(self, feats, confs, aflow, **kw):
        ...
        def clamp(xy):
            torch.clamp(xy[0], 0, W-1, out=xy[0]) ## W-1
            torch.clamp(xy[1], 0, H-1, out=xy[1]) ## H-1
            return xy
        
        # compute positive scores
        xy2p = clamp(xy2[:,None,:] + self.pos_offsets[:,:,None].to(xy2.device))
        # xy2p = clamp(xy2[:,None,:])
...
#        pscores = (feat1[None,:,:] * feat2p[b1,:,xy1p[1], xy1p[0]]).sum(dim=-1).t()
        if self.maxpool_pos:
            ...
            if confs: 
                sel = clamp(xy2 + self.pos_offsets[:,pos.view(-1)].to(xy2.device))
                qconf = (qconf + conf2[b2, :, sel[1], sel[0]].view(shape))/2
        
        # compute negative scores
        xy2n = clamp(xy2[:,None,:] + self.neg_offsets[:,:,None].to(xy2.device))
